...
def main() -> int:
    ...
    #    survive a copy-paste into a deployed environment.
    for required in DEPLOYMENT_REQUIRED:
        ...
        hits = [
            ...
            if required.local_default in path.read_text(encoding="utf-8")
            and path.name not in {"config.py", "deployment.py"}
        ]
        if hits:
            problems.append(
