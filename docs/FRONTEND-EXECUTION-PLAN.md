...
> **Both clauses are executed rather than reviewed, and that is the whole change
> M12 makes.** `frontend/tests/reconciliation.test.ts` carries an evidence entry
> for every §E28 row naming the tests, source and artefacts behind it; every named
> path must exist and be non-empty, every REAL row must name a test or an
> artefact, and a **finished** row — REAL over REAL — may name no source file that
> renders `<NotWired>`, `<FixtureNotice>` or `<ContractGap>`, and none that lives
> in the roadmap directory. The console's own `SCREENS` registry is cross-checked
> against the table in both directions: a screen's `wiring` and its row's `data`
> column must say the same thing about the same backend, and every fixture screen
> must be claimed by a row that does not call it REAL.
>
> The ledger is long on purpose. **A row nobody had to write evidence for is a
> row nobody checked** — which is, in one sentence, how three of this phase's five
> findings survived five milestones of review.
...
| **M9 — The Walk** | **Done** | §E16's nine acts ship on one damped `t ∈ [0,1]`: a Lenis proxy at `lerp 0.075`, a **Theatre.js** camera whose fourteen keys are authored as source and generated into a drift-checked project state (**ADR-0055**), GSAP's ScrollTrigger on the DOM, and scroll-snap per act. Gated by **26 browser assertions** in `tests/story.spec.ts` plus 36 unit assertions across four files. The two clauses that decide whether this is a film or a demo are both taken: **scroll controls distance travelled, not playback** — the scroll is held still for two seconds of frames and the walked distance is asserted unchanged, in the browser and again as a property of the damping in `tests/story-spine.test.ts` — and **Act 4 mounts the citizen loop's own `<ReportFlow>`**, asserted by comparing what the film renders against what `/report` renders rather than by looking at it. Act 5's five gates are stamped from a complaint filed *through the film*, read off its own ledger by M5's own reader. **Ten golden images** at a fixed `t`, seed, step and sun — nine acts and the storyboard. Every tier forced: Tier C by `prefers-reduced-motion` and again by `?tier=C`, Tier D with JavaScript switched off and all ten acts' copy still present, `axe` clean in both scripts on both. §E11.1's five motions are **audited as five** by a test that reads every stylesheet in `src/` and fails on a duration or curve that is not a token. **One clause is unexercised and published rather than skipped quietly:** Act 6 asserts that it renders nothing until a real `cluster_match_found` arrives, and the case that would watch one arrive skips by name because the committed fixture scores 0.142 against a 0.150 classifier floor and parks before dedup — [`docs/reports/story-merge-gate.md`](reports/story-merge-gate.md). **Five defects found and fixed**, three of them outside this milestone: an unreachable control plane 500ed every non-source locale (`loadStrings` caught the error *response* and not the thrown connection, contradicting its own docstring since M2); Act 4 threw on the server render because a component reused across surfaces brought a `QueryClientProvider` requirement with it; a device probe that throws held the film at *undecided* instead of falling to the storyboard; a pinned film re-snapped its own scroll; and two pre-existing type errors were red on a clean typecheck. **M9.6 — the characters — landed at F15 rather than here**, with the milestone's own ship line and Stage 4's row both saying so: see §E28's character layer row and **ADR-0048** |
| **M10 — Sound & the ladder** | **Done** | §E12's Web Audio graph ships **with no audio files in the repository** — **ADR-0050**: every cue is synthesised at runtime from a seeded generator that is itself the committed artefact, which is the same generate-don't-commit rule ADR-0047 and ADR-0055 already set, and the cost it pays is stated rather than hidden (*synthesised paper is not paper*). Four buses under a master gain, the modal duck counted rather than toggled, muted by default with the unmute **designed rather than hidden** (§E2 defect #9) and persisted. The ambient bed is cross-faded on the **same solar position the scene is lit by**, so the city cannot sound like morning while it looks like dusk (§E7.4). §E12's six interaction cues are each wired at exactly one call site, and the merge cue and the struck note are bound to `cluster_match_found` and `safety_trigger_fired` on the bus — there is no `playMerge()` for a button to call. **§E3.4 is now a grep rather than a review pass**: three new guards in `check-guards.ts` (`single-meaning-bloom`, `-stamp`, `-severity`) fail the build on a second use, each with a written allowlist — eight bans in that file now, up from four. **The ladder is proved rung by rung** in `tests/ladder.spec.ts`, against `capabilitiesFor()` and `pressQualityFor()` rather than against a transcription of the table, with the scene publishing what it actually built (`data-clay-press`, `data-clay-effects`); S and A are asserted as one forced trigger because ADR-0037 makes the backend the renderer's choice. **27 sound assertions** plus 6 ladder cases. **Two defects found and fixed, both by the gates:** a looping ambient bed carried 400 ms of silence at its seam — the cross-fade folded the tail in and then kept it, so the city would have gone quiet once a lap, forever; and `SoundGraph.state()` built a fresh object on every call, which through `useSyncExternalStore` is an infinite render loop, and the browser reported it as the unmute button *detaching from the DOM* until the case timed out. **One clause is published as unbuilt rather than faked:** positional foley is implemented and asserted as a pure function and is **wired to nothing**, because the only positioned data this frontend receives is wards — [`docs/reports/positional-foley-gap.md`](reports/positional-foley-gap.md) |
| **M11 — Field & offline** | **Done** | `/field` ships: three sections, a large camera button, and no board anywhere (§E21's *field staff never see a kanban*). **ADR-0056** — the offline queue is a durable outbox in IndexedDB **keyed by the idempotency key**, which makes a duplicate submission unrepresentable rather than merely unlikely, and the queue is the **only** writer of a submission online or off, so the durability path is the path a perfect connection takes. **ADR-0057** — compression preserves EXIF by transplanting the original's `APP1` segments into the re-encoded file, byte for byte, with a walker that copies and never parses; anything it does not fully understand is sent **uncompressed and intact** rather than compressed-and-stripped, and GPS is carried deliberately with the argument written down. **Outdoor mode is a ground, not a theme**: `data-ground="outdoor"` re-points every role token at a third palette whose every floor is **7:1**, generated by the same generator and checked by the same contrast loop that already ran over two grounds — so Phase 22's contrast clause is a build failure rather than a report. PWA with a generated maskable icon (no committed binaries, again) and a hand-written service worker that is **network-first for navigations and never touches `/api/*`**. **The Phase 22 gate is taken against the live stack**: a report captured with the network switched off, the app reloaded *still offline* — the shell served by the worker — the submission found intact in the store, and then filed exactly once on reconnect; a second drain files nothing further; the flow runs on a 500 ms / 50 kbps 2G profile. **17 unit assertions** plus 8 browser cases. **Two limits are stated rather than smoothed over:** the job *list* is behind the §E24 chip because `openapi.json` has no work-order schema (Phase 14), and it draws no fixture jobs at all — a list of three invented jobs on the surface whose whole purpose is *do this next* is the one place a fixture would be indistinguishable from a lie; and the offline reload renders the queue correctly on the **production** build but not on the dev server, which serves the shell and cannot hydrate it — measured, not assumed, and the gate reads the store rather than the bundler |
| **M12 — Reconciliation** | **Done** | Track E closes by executing the two tables it has been graded against, rather than reading them — and **five claims were wrong, none found by reading**. §44's *underreporting-zone equity flag* read **REAL** and is rendered in exactly one place in this product: `console/roadmap/AreaView.tsx`, behind the §E24 chip, beside a `<ContractGap>` naming the endpoint that does not exist — **a REAL row backed by a fixture**, the literal thing M12.5 gates against, sitting in the master table since v2.0 and corrected in §44 and in §23.2. §44's *public read-only transparency API* read **ROADMAP** and every endpoint §26.4 describes shipped at M6 — understating is still drift and is the direction nobody audits for; corrected in three places. §E28 carried *golden images, Storybook diffs, Lighthouse, WCAG audit, usability session* as **one** ROADMAP row, understating the three F1 and F3 shipped and hiding the two that had not happened; split. And §E27, executed for the first time, found **two registered audit events that reach no surface at all** (`evaluation_set_retired`, `policy_certification_waived` — now register row **A18**, owned by Phase 17) and **three facts the evidence trail has rendered since M5 sitting under bare *(Phase 14)* / *(Phase 15)* notes**. Gated by **98 reconciliation assertions** plus the traceability check in `nem check`: every §E28 row names evidence that exists, every REAL row traces to a test or an artefact, no finished row draws a fixture, and every console screen's `wiring` and its row's `data` column are asserted to say the same thing about the same backend. **A17 decided rather than deferred again** — ADR-0058 removes the string tier that could never resolve, held by a ninth guard, a test asserting the loader makes no upstream call, and a deleted proxy; the first version of that test passed with the tier pasted back in and was rewritten until it failed. **A15 and A16 are carried, not closed**: the WCAG audit and the usability session are people, nobody is booked, the risk was named at plan time as a lead-time item to start at F3, and it was not started — so the phase publishes the instruments and the fact ([`wcag-audit-gap.md`](reports/wcag-audit-gap.md), [`usability-session-gap.md`](reports/usability-session-gap.md)) rather than a number |
...
| ~~A8~~ **Closed — F1** | ~~No golden images~~ — baselines for every surface at fixed seed, camera, step and sun, animations disabled, `deviceScaleFactor: 1`, captured in CI's own container image because a PNG rendered by a Windows font stack and one rendered by Ubuntu's differ in every antialiased pixel and the comparison is at zero tolerance. Each one **verified to fail** against a deliberately perturbed render — a baseline nobody has watched fail is a screenshot, not a gate. Ten more landed at F14 for the Walk's nine acts and its storyboard. **The row was never struck through when F1 closed it**, and F18 is where that was noticed | §E24, and the Phase 20 gate | — |
| ~~A9~~ **Closed — F1** | ~~Storybook is built, not diffed~~ — `scripts/storybook-diff.ts` compares the catalogue against the base ref and fails the build on an unreviewed visual change, rather than uploading an artefact a reviewer has to think to open | §E24: *"Every visual PR posts its Storybook diff and a five-second scene capture"* | — |
...
| A18 **Owned by Phase 17** | **Two audit events reach no surface at all.** `evaluation_set_retired` and `policy_certification_waived` are registered, shipped and on the chain; neither appears anywhere in `frontend/src/` outside the generated client. Their own docstrings say what they are for — *"which activations bypassed the evaluation set, and on what grounds"*, and *"a chain that recorded the second and not the first would let the interesting half of an incident happen off the record"* | §E27, §6 Principle #9 | **Found at F18 by executing §E27 rather than reading it.** Not a rendering bug: a gap between what the log records and what any human can see, on the one screen where it matters most — the policy studio is REAL over REAL and renders the guardrail's *refusal* while saying nothing about a guardrail switched off. Owned by **Phase 17**, whose integrity room and case file are where an audit view lands; Track E cannot claim a screen no F-phase was scoped to build, and drawing one at F18 would be building product in the reconciliation phase and calling it reconciliation. §E27 carries the gap as an explicit row. [`docs/reports/e27-audit.md`](reports/e27-audit.md) |
...
| ~~A17~~ **Closed — F18, ADR-0058** | **The control plane cannot carry product copy, and the frontend implied it could.** `loadStrings` resolves three tiers — base, seed, control plane — and the third fetches `/api/v1/control-plane/translations/{namespace}/{locale}` for `common`, `citizen`, `console` and `public`. The registry carries none of them: `db/models/i18n.py` lists `taxonomy`, `organisation`, `zone` and `calendar`, and an import into `public` is refused. So that tier resolves to `{}` on every request it will ever make | §E10.1, §E22, §E25 Phase 18 | Found by writing A2's gate, which is the argument for writing gates. **Nothing is broken and nothing renders wrong** — the seed bundles carry the words and the merge degrades exactly as designed — but a tier that cannot ever return anything is a capability the code claims and does not have, and §E3.3 is a rule about precisely that. The decision is not a patch: either product copy becomes tenant-importable, which reopens *"a tenant could overwrite the wording of a legal notice"* that `db/models/i18n.py` settled — weakened, but not answered, by C7 moving the §22.2 notices into code — or the frontend stops implying it can and the tier is removed. F18 is where the honesty table is verified line by line, which is where a claim the product cannot support belongs. **Decided: the tier goes** (ADR-0058). Widening the registry was the other honest answer and is argued and rejected there — it reopens *"a tenant could overwrite the wording of a legal notice"*, which ADR-0052 weakened by moving the §22.2 notice and the §16.1 disclaimer into code and did not answer, because the §22.1 consent text and every refusal sentence are still product copy. Three things hold it: a ninth guard in `check-guards.ts` fails the build on the endpoint literal, `tests/strings.test.ts` asserts the loader makes **no upstream call at all** — the stronger property, and the first version of that test was decoration until it was watched failing — and the unused BFF proxy at `app/api/i18n/` is deleted. Phase 18's gate is untouched: it governs the tenant-authored half, and `nem gate-phase18-locale` asserts that end to end over HTTP |
| ~~A13~~ **Closed — F1** | ~~The no-off-origin assertion covers fonts only~~ — `tests/fixtures/origin.ts` watches **every** request a page makes, and `tests/origin.spec.ts` names the routes it watches: images, scripts, audio and XHR, on the offline-capable surfaces and the live ones alike. The route list is explicit rather than crawled — a page that a dependency reaches out from is as likely to be an unlinked proof surface as a public one — and a final case asserts every scripted surface is covered by the sweep, so adding a route without adding it here fails rather than passing quietly | §6 Principle #6, §E24 | — |
...
| A15 | **WCAG 2.2 AA verified by audit, not only by automated scan** | §E25 Phase 18, §E22 | **Open at the close of Track E, and instrumented rather than closed.** The scan half is done, clean and broad — `axe` at `wcag2a/2aa/21a/21aa/22aa` over the console at three densities × two scripts and over the open palette, all seven fixture screens, the citizen, public and story surfaces in both scripts, the §E26 matrix at all twelve combinations, plus 48 token contrast assertions, a third role ground at 7:1, and Lighthouse asserting 100 accessibility on three routes. By `axe`'s own account that is about a third of the standard. **The audit half is a person and it has not happened.** F18 wrote the instrument instead of faking the outcome: all 56 AA criteria dispositioned `automated` / `gated` / `person`, and three items flagged as expected-to-fail in advance — including WCAG 2.2's new **2.5.7 Dragging Movements** against the clay camera, which the author expects to need a single-pointer alternative. [`docs/reports/wcag-audit-gap.md`](reports/wcag-audit-gap.md) |
| A16 | **Measured task-success rate from a usability session** with real field staff and department users, findings tracked | §E25 Phase 18 — *"the plan calls this a design practice, not a component library, and that distinction is the gate"* | **Not run. No participant has ever touched this product**, and that is the largest unquantified risk in this frontend — larger than any technical row here, because every technical row is a thing somebody knows is missing. Three ways to produce a number anyway were available and all three rejected, for the reason each is written down: colleagues are not the users, the author is a rehearsal, and reporting the E2E pass rate as task success is the exact category error §E28 was rewritten to remove. The protocol is written — ten tasks with binary criteria fixed in advance, field sessions outdoors on the participant's own phone, and **no pass mark set**, deliberately, because a threshold chosen by the builder before any data exists is a threshold chosen to be cleared. [`docs/reports/usability-session-gap.md`](reports/usability-session-gap.md) |
...
- **M9.6** Characters as event-driven state machines — **landed at F15**, and **ADR-0048** amends the renderer: §E8.1 specifies named inputs and states, not `.riv`, so the contract is the machine and Rive becomes a swappable renderer the day a file exists
...
- **M10.2** Positional foley — **built and unmounted**; the only positioned data this frontend receives is wards, and choosing a defect loop from a ward's name would be a sound asserting a fault nobody detected ([`positional-foley-gap.md`](reports/positional-foley-gap.md))
- **M10.3** The merge cue
...
> **Five wrong claims, and none of them found by reading.** The gate is now
> executed: `frontend/tests/reconciliation.test.ts` (98 assertions) requires every
> §E28 row to name evidence that exists, every REAL row to trace to a passing test
> or a shipped artefact, and no finished row to be drawn by `<NotWired>`,
> `<FixtureNotice>` or `<ContractGap>`; `scripts/check_surface_traceability.py`
> executes §E27 in both directions against the event catalog and against
> `frontend/src/`. Full record: [`docs/reports/m12-reconciliation.md`](reports/m12-reconciliation.md)
> and [`docs/reports/e27-audit.md`](reports/e27-audit.md).

- **M12.1** §E28 and §44 verified line by line
- **M12.2** §E27 audited
- **M12.3** The usability session (A16) — **not run, and the ship line says so rather than the phase claiming it.** There are no participants: a solo build with no pilot partner has no field staff and no department users. What F18 shipped instead is the instrument — ten tasks with binary success criteria fixed in advance, field sessions outdoors on the participant's own phone, and no pass mark set. A16 is **carried** by F18, not closed by it
